...
export const StudentDashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const myTickets = tickets.filter(t => t.submittedBy === user.id);
  const pendingCount = myTickets.filter(t => t.status === 'Pending').length;
  const resolvedCount = myTickets.filter(t => t.status === 'Resolved').length;

  // Filter appointments
  const myAppointments = myTickets.filter(t => t.appointmentDate);

  // Filter notifications for current student
  const myNotifications = notifications.filter(n => n.recipientId === user.id).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  ...
  const handlePrintSlip = (ticket: TicketType | null = selectedTicket) => {
    ...
    // Generate mock reference code: REF-{TicketID}-{Random}
                    ...
                    <div className="max-w-6xl mx-auto space-y-8 pb-10">
                        ...
                        <div className="grid gap-4">
                            {paginatedTickets.map((ticket) => (
                                <div key={ticket.id} className="group bg-white rounded-xl p-5 border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 hover:border-emerald-300 cursor-pointer" onClick={() => setSelectedTicket(ticket)}>
                                            ...
                                            <table className="w-full text-sm text-left">
                                                ...
                                                <tbody className="divide-y divide-slate-100 bg-white">
                                                    {myTickets.slice(0, 5).map((ticket) => (
                                                        <tr key={ticket.id} className="hover:bg-slate-50 cursor-pointer transition-colors" onClick={() => setSelectedTicket(ticket)}>
                        ...
                        <Card className="border border-slate-200 shadow-sm rounded-xl overflow-hidden bg-white">
                            ...
                            <CardContent className="p-4 space-y-4">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-sm font-medium text-slate-900">{selectedTicket.lastUpdated}</span>
                                </div>
                            </CardContent>
                        </Card>
