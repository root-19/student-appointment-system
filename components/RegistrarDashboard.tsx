...
export const RegistrarDashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const getStudentTickets = (studentId: string) => {
    return tickets.filter(t => t.submittedBy === studentId && t.category === 'Registrar');
  };
  ...
  const MOCK_WEEKLY_DATA = [
      ...
      { name: 'Tue', tickets: 6, resolved: 3 },
      { name: 'Wed', tickets: 8, resolved: 5 },
      { name: 'Thu', tickets: 5, resolved: 4 },
      { name: 'Fri', tickets: 9, resolved: 6 },
  ];
